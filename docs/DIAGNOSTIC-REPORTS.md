...
The development branch and lab candidate do not advance either published plugin feed during Community Applications review. Existing protocol 5 operations and the negotiated recovery plane are unchanged; diagnostic-host is a read-only optional command, with graceful fallback on older peers.
